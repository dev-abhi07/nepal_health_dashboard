...
exports.dashboard = async (req, res) => {
  try {
    ...
    if (province_id) {
      const [totalDistricts, totalPalikas, totalWards] = await Promise.all([
        district_master.count({
          where: { isdeleted: 0, fk_provinceid: province_id },
        }),
        palika_master.count({
          where: { isdeleted: 0, fk_provinceid: province_id },
        }),
      ...
      demography = [
        { title: "Provinces", total_count: 1 },
        { title: "Districts", total_count: totalDistricts },
        { title: "Palikas", total_count: totalPalikas },
        { title: "Wards", total_count: totalWards },
      ];

      totalFacility = authority_level
        ? await Helper.getProvincesFacilityCountsAuthorityLevel(
          province_id,
          authority_level
        )
        : await Helper.getProvincesFacilityCounts(province_id);

      childrenZeroTo14Years = await sequelize.query(
        "SELECT SUM(pop00to14years) FROM population_stats WHERE province_id = :province_id",
        { type: sequelize.QueryTypes.SELECT, replacements: { province_id } }
      );
      childrenZeroTo5years = await sequelize.query(
        "SELECT SUM(pop00to59months) FROM population_stats WHERE province_id = :province_id",
        { type: sequelize.QueryTypes.SELECT, replacements: { province_id } }
      ...
      const [totalPalikas, totalWards] = await Promise.all([
        palika_master.count({
          where: { isdeleted: 0, fk_districtid: district_id },
        }),
        WardMaster.count({
          where: { isdeleted: 0, fk_districtid: district_id },
        }),
      ...
      demography = [
        { title: "Provinces", total_count: 1 },
        { title: "Districts", total_count: 1 },
        { title: "Palikas", total_count: totalPalikas },
        { title: "Wards", total_count: totalWards },
      ];

      totalFacility = authority_level
        ? await Helper.getDistrictFacilityCountsAuthorityLevel(
          district_id,
          authority_level
        )
        : await Helper.getDistrictFacilityCounts(district_id);

      childrenZeroTo14Years = await sequelize.query(
        "SELECT SUM(pop00to14years) FROM population_stats WHERE district_id = :district_id",
        { type: sequelize.QueryTypes.SELECT, replacements: { district_id } }
      );
      childrenZeroTo5years = await sequelize.query(
        "SELECT SUM(pop00to59months) FROM population_stats WHERE district_id = :district_id",
        { type: sequelize.QueryTypes.SELECT, replacements: { district_id } }
      ...
      const totalWards = await WardMaster.count({
        where: { isdeleted: 0, fk_palikaid: palika_id },
      });

      demography = [
        { title: "Provinces", total_count: 1 },
        { title: "Districts", total_count: 1 },
        { title: "Palikas", total_count: 1 },
        { title: "Wards", total_count: totalWards },
      ];

      totalFacility = authority_level
        ? await Helper.getPalikaFacilityCountsAuthorityLevel(
          palika_id,
          authority_level
        )
        : await Helper.getPalikaFacilityCounts(palika_id);

      childrenZeroTo14Years = await sequelize.query(
        "SELECT SUM(pop00to14years) FROM population_stats WHERE municipality_id = :municipality_id",
        {
          type: sequelize.QueryTypes.SELECT,
          replacements: { municipality_id: palika_id },
        }
      );
      childrenZeroTo5years = await sequelize.query(
        "SELECT SUM(pop00to59months) FROM population_stats WHERE municipality_id = :municipality_id",
      ...
      const [
        ...
        province_master.count({ where: { isdeleted: 0 } }),
        district_master.count({ where: { isdeleted: 0 } }),
        palika_master.count({ where: { isdeleted: 0 } }),
        WardMaster.count({ where: { isdeleted: 0 } }),
        facilitytypemaster.findAll({
          where: { isdeleted: 0 },
          order: [["facilitytype", "ASC"]],
        }),
      ]);
      ...
      demography = [
        { title: "Provinces", total_count: province_masters },
        { title: "Districts", total_count: totalDistricts },
        { title: "Palikas", total_count: totalPalikas },
        { title: "Wards", total_count: totalWards },
      ];

      totalFacility = await Promise.all(
        facilityTypes.map(async (r) => {
          ...
          if (r.facilitytype == "Warehouse") {
            count = await VaccineWarehouse.count({});
          }
          ...
          return {
            ...
            facility_type: r?.facilitytype,
            id: r.id,
            facility_image: `icons/${r.image || null}`,
            color_code: r.color_code,
          };
        })
      );
    ...
    totalDifference =
      childrenZeroTo14Years[0].sum - childrenZeroTo5years[0].sum;
    ...
    populationData = {
      ...
      legends: [
        {
          label: `Upto 5 Years ${Number(
            childrenZeroTo5years[0].sum
          ).toLocaleString("en-IN")}`,
          dotColor: "#dc143c",
        },
        {
          label: `5-14 Years ${Number(totalDifference).toLocaleString(
            "en-IN"
          )}`,
          dotColor: "#003893",
        },
  ...
  upload.single("uploaded_file")(req, res, async (err) => {
    ...
    for (const row of rows) {
      await vaccine_hmis.create({
        ...
        dropout_dpt1_vs_3: parseFloat(
          row["6.20 - DPT-HepB-Hib dropuout rate (DPT-HepB-Hib 1 vs 3)"]
        ),
        dropout_dpt1_vs_mr2: parseFloat(
          row["6.21 - DPT-HepB-Hib1 vs MR2 dropout rate"]
        ),
        dropout_pcv1_vs_3: parseFloat(
          row["6.22 - PCV dropout rate (PCV1 vs PCV3)"]
        ),
        dropout_mr: parseFloat(row["6.23 - Measles/Rubella droupout rate"]),
        wastage_bcg: parseFloat(row["6.25 a - Vaccine wastage rate (BCG)"]),
        wastage_dpt_hepb_hib: parseFloat(
          row["6.25 b - Vaccine wastage rate (DPT/HepB/Hib)"]
        ),
        ...
        pct_sessions_conducted: parseFloat(
          row["6.26 - % of planned immunization sessions conducted"]
        ),
        pct_clinics_conducted: parseFloat(
          row["6.27 - %Â of planned immunization clinics conductedÂ "]
        ),
        pct_serious_aefi: parseFloat(
          row["6.30 - % of serious AEFI among reported AEFI cases"]
        ),
      });
    }
...
exports.vaccineDropChart = async (req, res) => {
  try {
      ...
      const parts = [
        {
          [Op.and]: [
            { year: S.year },
            { month: { [Op.in]: MONTHS.slice(S.monthIdx) } },
          ],
        },
        {
          [Op.and]: [
            { year: E.year },
            { month: { [Op.in]: MONTHS.slice(0, E.monthIdx + 1) } },
          ],
        },
      ];
      if (E.year - S.year > 1) {
        parts.splice(1, 0, {
          year: { [Op.between]: [S.year + 1, E.year - 1] },
        });
      }
    ...
    const monthOrderLiteral = `array_position(ARRAY['January','February','March','April','May','June','July','August','September','October','November','December']::text[], "month"::text)`;
    let rows = []
    if (req?.body?.facility_code) {
      const query = `
      select vh.year,vh.month,vh.wastage_bcg,vh.wastage_dpt_hepb_hib,vh.wastage_fipv,vh.wastage_je,vh.wastage_mr,vh.wastage_opv,vh.wastage_rota,vh.wastage_tcv,vh.wastage_td,vh.wastage_pcv
from facilitycodemaster as fcm join vaccine_hmis as vh on fcm.facilitycode = vh."nhfrId"
where fcm.facilitycode = :facility_code 
order by vh.year,vh.month
      `
      rows = await sequelize.query(query, {
        replacements: { facility_code: req.body.facility_code },
        type: Sequelize.QueryTypes.SELECT
      })
      ...
      select vh.year,vh.month,vh.wastage_bcg,vh.wastage_dpt_hepb_hib,vh.wastage_fipv,vh.wastage_je,vh.wastage_mr,vh.wastage_opv,vh.wastage_rota,vh.wastage_tcv,vh.wastage_td,vh.wastage_pcv
from facilitycodemaster as fcm join vaccine_hmis as vh on fcm.facilitycode = vh."nhfrId"
where fcm.id = :facility_code 
order by vh.year,vh.month
      `
      rows = await sequelize.query(query, {
        replacements: { facility_code: req.body.id },
        type: Sequelize.QueryTypes.SELECT
      })
      ...
      rows = await vaccine_hmis.findAll({
        attributes,
        where,
        order: [
          ["year", "ASC"],
    ...
    for (const r of rows) {
      ...
      metrics.forEach((k) => {
        acc.sum[k] = (acc.sum[k] || 0) + safe(r[k]);
      });
      acc.cnt += 1;
      bucket.set(key, acc);
    }
    ...
    const lineChart = {
      ...
      title: {
        ...
        align: "left",
        style: { fontWeight: "700", fontSize: "16.8px", color: "#364a36", fontFamily: "Roboto, sans-serif" },
      },
    ...
    return Helper.response(
      true,
      "Vaccine Dropout, Wastage & KPI Chart",
      { lineChart },
      res,
      200
    );
...
exports.vaccineWarehouseChart = async (req, res) => {
  try {
    const categories = [
      "January 2024",
      "February 2024",
    ...
    if (req.body?.province_id) {
      query = `
    SELECT 
  SUM(a.total_stock) AS total_stock_sum,
  a.vaccine_name,
  a.month,
  a.year
FROM vaccine_storage as a
join vaccine_warehouse as b on a.storage_center_id = b.id
WHERE a.month IN ('February', 'March','May','June','July') and b.province_id = '${req.body.province_id}'
AND vaccine_name IN (${vaccinePlaceholders})
GROUP BY a.vaccine_name, a.month, a.year
    `
...
FROM vaccine_storage as a
join vaccine_warehouse as b on a.storage_center_id = b.id
WHERE a.month IN ('February', 'March','May','June','July') and b.district_id = '${req.body.district_id}'
AND vaccine_name IN (${vaccinePlaceholders})
GROUP BY a.vaccine_name, a.month, a.year
    `
...
FROM vaccine_storage as a
join vaccine_warehouse as b on a.storage_center_id = b.id
WHERE a.month IN ('February', 'March','May','June','July') and b.palika_id = '${req.body.palika_id}'
AND vaccine_name IN (${vaccinePlaceholders})
GROUP BY a.vaccine_name, a.month, a.year
    `
   ...
   SELECT
      ...
      WHERE month IN ('February', 'March','April','May','June','July')
        AND vaccine_name IN (${vaccinePlaceholders})
      GROUP BY vaccine_name, month, year;
  `;
    ...
    // Create month-year label index map
    const monthYearIndex = categories.reduce((obj, label, idx) => {
      obj[label] = idx;
      return obj;
    }, {});

    // Group results by vaccine_name
    const vaccineGroups = {};

    // Initialize vaccineGroups with empty data arrays
    results.forEach(row => {
      if (!vaccineGroups[row.vaccine_name]) {
        vaccineGroups[row.vaccine_name] = new Array(categories.length).fill(0);
      }
    });

    results.forEach(row => {
      const label = `${row.month} ${row.year}`;
      const idx = monthYearIndex[label];
      if (idx !== undefined) {
        vaccineGroups[row.vaccine_name][idx] += Number(row.total_stock_sum || 0);
      }
    });

    // Prepare series array from grouped data
    const series = Object.entries(vaccineGroups).map(([vaccineName, dataArray], idx) => ({
      name: vaccineName,
      data: dataArray,
      visible: idx < 3 // 
    }));
    ...
    const lineChart = {
      chart: { type: "column", height: 445 },
      title: {
        text: "Monthly Vaccine Warehouse Report",
        align: "left",
        style: {
          fontWeight: "700",
          fontSize: "16.8px",
    ...
    // Return response
    return Helper.response(true, "Vaccine Warehouse Chart", { lineChart }, res, 200);

  } catch (error) {
    return Helper.response(false, "Failed to get data", null, res, 500);
  }
};
...
exports.ImmunizationRecord = async (req, res) => {
  try {
    ...
    const config =
      programConfig[selectedProgram] || programConfig["measles/rubella"];
    ...
    let rows = []
    if (req?.body?.facility_code) {
      const sumAttributesRaw = config.columns.map((col) =>
        `COALESCE(SUM("${col}"), 0) AS sum_${col}`
      );
      const query = `
      select ${sumAttributesRaw.join(", ")},vh."time_period" from facilitycodemaster as fcm join immunization_records as vh on fcm.facilitycode = vh."nhfr_id"
where fcm.facilitycode = :facility_code  group by vh."time_period"
      `
      ...
      const query = `
      select ${sumAttributesRaw.join(", ")},vh."time_period" from facilitycodemaster as fcm join immunization_records as vh on fcm.facilitycode = vh."nhfr_id"
where fcm.id = :facility_code  group by vh."time_period"
      `
      ...
      rows = await ImmunizationRecord.findAll({
    ...
    const series = config.columns.map((col, idx) => ({
      name: col
        .toUpperCase()
        .replace(/_/g, " ")
        .replace(/^MR 9 11M$/i, "MR1")
        .replace(/^MR 12 23M$/i, "MR2"),
      data: keySeq.map((k) => {
        const rec = bucket.get(k);
        return rec ? Number(rec[`sum_${col}`]) || 0 : 0;
      }),
...
exports.ImmunizationRecordByProgramName = async (req, res) => {
  try {
    ...
    return Helper.response(
      false,
      "Error fetching immunization record by program name",
      {},
      res,
      500
    );
...
exports.populationChart = async (req, res) => {
  try {
    ...
    data = [
      {
        name: "Children Below 5",
        y: parseFloat(childrenBelow5[0].sum),
        color: "#0fac81",
      },
      {
        name: "Adolescent Girls",
        y: parseFloat(adolescentGirls[0].sum),
        color: "#dc143c",
      },
      {
        name: "Woman of Child <br>Bearing Age",
        y: parseFloat(WomanOfChildbearingAge[0].sum),
        color: "#213dad",
      },
    ];
...
exports.DashboardTableData = async (req, res) => {
  try {
    ...
    if (req.body && req.body.province_id) {
      ...
      if (req.body?.authority_level) {
        rawData = await Helper.getProvienceTableDataAuthorityLevel(
      ...
      const query = `
    select a.district_id,b.districtname,count(a.id) from vaccine_warehouse as a join districtmaster as b on b.districtid = a.district_id 
where a.province_id = :provinceid
group by a.district_id,b.districtname
    `


      const warehouseData = await sequelize.query(query, {
        type: sequelize.QueryTypes.SELECT,
        replacements: { provinceid: req.body.province_id },
      });
      ...
      rawData.forEach((row) => {
        ...
        districtMap[row.districtid][row.facilitytype] = parseInt(
          row.total_facilities,
          10
        );
      ...
      warehouseData.forEach((row) => {
        if (!districtMap[row.district_id]) {
          districtMap[row.district_id] = {
            Name: row.districtname,
            Total: 0,
          };
          facilityTypeKeys.forEach((key) => {
            districtMap[row.district_id][key] = 0;
          });
          districtMap[row.district_id].districtid = row.district_id;
        }

        districtMap[row.district_id]["Warehouse"] = parseInt(row.count, 10);
        districtMap[row.district_id].Total += parseInt(row.count, 10);
      });
      ...
      if (req.body?.authority_level) {
        rawData = await Helper.getDistrictTableDataAuthorityLevel(
...
select a.palika_id,b.palikaname,count(*) from vaccine_warehouse as a join palikamaster as b on b.palikaid = a.palika_id
where a.district_id = :district_id
group by a.palika_id,b.palikaname
      ...
      rawData.forEach((row) => {
        ...
        palikaMap[row.palikaid][row.facilitytype] = parseInt(
          row.total_facilities,
          10
        );
      ...
      warehouseData.forEach((row) => {
        if (!palikaMap[row.palika_id]) {
          palikaMap[row.palika_id] = {
            Name: row.palikaname,
            palika_id: row.palika_id,
            Total: 0,
          };
          facilityTypeKeys.forEach((key) => {
            palikaMap[row.palika_id][key] = 0;
          });
          palikaMap[row.palika_id].palika_id = row.palika_id;
        }

        palikaMap[row.palika_id]["Warehouse"] = parseInt(row.count, 10);
        palikaMap[row.palika_id].Total += parseInt(row.count, 10);
      });
      ...
      if (req.body?.authority_level) {
        rawData = await Helper.getPalikaTableDataAuthorityLevel(
          req.body.palika_id,
          req.body.authority_level
        );
      } else {
      ...
      rawData.forEach((row) => {
        if (!wardMap[row.wardid]) {
          ...
          facilityTypeKeys.forEach((key) => {
            wardMap[row.wardid][key] = 0;
          });
        }

        wardMap[row.wardid][row.facilitytype] = parseInt(row.total_facilities, 10);
        wardMap[row.wardid].Total += parseInt(row.total_facilities, 10);
        wardMap[row.wardid].wardid = row.wardid;
      });
      ...
      tableData = await Promise.all(
        provinces.map(async (province) => {
          ...
          if (req.body?.authority_level) {
            // 🏭 Case 1: Normal facilities with authority filter
            ...
            // 🏭 Case 2: Normal facilities without authority filter
            facilities = await facility.findAll({
              where: {
                fk_provinceid: province.provinceid,
                isdeleted: 0,
          ...
          // 📦 Case 3: Warehouse facilities (from vaccine_warehouse instead of facility)
          const warehouseData = await sequelize.query(
            `
              SELECT 
                a.province_id, 
                b.province, 
              ...
              JOIN provincemaster AS b ON b.provinceid = a.province_id
              WHERE a.province_id = :provinceid
              GROUP BY a.province_id, b.province
            `,
            {
              replacements: { provinceid: province.provinceid },
              type: sequelize.QueryTypes.SELECT,
            }
          );
          ...
          if (warehouseData.length > 0) {
            typeCounts["Warehouse"] = warehouseData[0].warehouse_count;
          }

          let row = {
            Name: province.province,
            provinceid: province.provinceid,
            Total: facilities.length + parseInt(typeCounts["Warehouse"] || 0),
          };
  ...
  try {
    ...
    if (req.body.province_id) {
      const result = await Promise.all(
        categories.map((cat) =>
          Helper.getHealthWorkerCategoryProvince(req.body.province_id, cat.id)
      ...
      const result = await Promise.all(
        categories.map((cat) =>
          Helper.getHealthWorkerCategoryDistrict(req.body.district_id, cat.id)
      ...
      const result = await Promise.all(
        categories.map((cat) =>
          Helper.getHealthWorkerCategoryPalika(req.body.palika_id, cat.id)
        )
      );
      data = categories.map((cat, idx) => ({
        title: cat.title,
    ...
    return Helper.response(
      false,
      "Error fetching facility type data",
      {},
      res,
      500
    );
...
exports.getfacilityByProvince = async (req, res) => {
  try {
    ...
    if (!facilityData || facilityData.length === 0) {
      return Helper.response(
        false,
        "No facility data found for the given province",
        {},
        res,
        200
      );
    ...
    return Helper.response(
      false,
      "Error fetching facility by province",
      {},
      res,
      500
    );
...
exports.getDistricts = async (req, res) => {
  try {
    ...
    const geograhy = {
      ...
      features: realGeojson.features.filter(
        (f) => f.properties && f.properties.PROVINCE === province_id
      ),
    };
    ...
    return Helper.response(
      false,
      "Error fetching district by province",
      {},
      res,
      500
    );
...
exports.getPalika = async (req, res) => {
  try {
    ...
    const geograhy = {
      type: realGeojson.type,
      features: realGeojson.features.filter(
        (f) => f.properties && f.properties.dID === parseInt(district_id)
      ),
    };
    return Helper.response(true, "Districts data", geograhy, res, 200);
  } catch (err) {
...
exports.getWard = async (req, res) => {
  try {
    ...
    const geograhy = {
      type: realGeojson.type,
      features: realGeojson.features.filter(
        (f) => f.properties && f.properties.aID == palika_id
      ),
    };
  ...
  try {
    ...
    if (req.body.province_id && req.body.cat_id) {
      emp_worker_data = await sequelize.query(
        `
  SELECT hwd.name as emp_name,hwd.gender,hwd.group,hwd.post,hwd.qualification,hwc.name,dm.districtname,fcm.facilityname,fcm.facilitycode FROM districtmaster dm
...
Join facilitycodemaster as fcm on fcm.id = hwd.fk_facilitycode
...
WHERE hwc.id = :cat and pms.palikaid = :palika_id`,
      ...
      emp_worker_data = await sequelize.query(
        `
  SELECT hwd.name as emp_name,hwd.gender,hwd.group,hwd.post,hwd.qualification,hwc.name,dm.districtname,fcm.facilityname,fcm.facilitycode FROM health_worker_data hwd
...
exports.vaccinewarehouseList = async (req, res) => {
  try {
    ...
    if (district_id) {
      whereCondition.district_id = district_id
    }
    if (palika_id) {
      whereCondition.palika_id = palika_id
    }

    const data = await VaccineWarehouse.findAll({
      where: whereCondition,
      raw: true,
      order: [["id", "desc"]],
    });
    if (data.length == 0) {
      return Helper.response(false, "No data found", data, res, 404);
    }
    ...
    const finaldata = await Promise.all(
      data.map(async (item) => {
        ...
        const province = await province_master.findOne({
          where: { provinceid: item?.province_id },
        });
        ...
        const palika = await palika_master.findOne({
          where: {
            palikaid: item?.palika_id
          },
        });
        const facility = await facilitytypemaster.findOne({
          where: {
        ...
        return {
          ...
          facility_name: facility?.facilitytype,
...
exports.wareHouseDataDD = async (req, res) => {
  try {

    const query = ` 
   SELECT "month", "year" FROM vaccine_storage group by "month","year";
    `
    const quaryData = await sequelize.query(query, { type: sequelize.QueryTypes.SELECT });
    const data = quaryData.map((item) => {
      return {
...
exports.vaccinewarehousebyId = async (req, res) => {
  try {
    const { id } = req.body;

    if (!id) {
      return Helper.response(false, "Please provide id", {}, res, 400);
    }

    const data = await VaccineWarehouse.findOne({
      where: { id },
    });
    ...
    const province = await province_master.findOne({ where: { provinceid: data.province_id } });
    newData['province_name'] = province?.province;

    const district = await district_master.findOne({ where: { districtid: data.district_id } });
    newData['district_name'] = district?.districtname;

    const palika = await palika_master.findOne({ where: { palikaid: data.palika_id } });
    newData['palika_name'] = palika?.palikaname;

    const facility = await facilitytypemaster.findOne({ where: { id: data.facilitytype_id } });
    newData['facility_name'] = facility?.facilitytype;
    newData['icon'] = facility?.image;
    newData['color_code'] = facility?.color_code;



    let vaccineDataQuery;

    vaccineDataQuery = `
    select vaccine_name,total_stock,usable_qty,opening_balance,total_received,total_issued from vaccine_storage where storage_center_id = :id
    `
    ...
    if (req.body?.month) {
      ...
      //       const [startMonthName, startYear] = req.body.start_date.split('-');
      //       const [endMonthName, endYear] = req.body.end_date.split('-');

      //       const startMonth = monthMap[startMonthName];
      //       const endMonth = monthMap[endMonthName];

      //       const startYYYYMM = `${startYear}${startMonth}`;
      //       const endYYYYMM = `${endYear}${endMonth}`;

      //       vaccineDataQuery = `
      //      SELECT vaccine_name, total_stock, usable_qty, opening_balance, total_received, total_issued
      // FROM vaccine_storage
      // WHERE storage_center_id = :id
      //   AND CONCAT("year", 
      //     LPAD(
      //       CASE 
      ...
      //      vaccineData = await sequelize.query(vaccineDataQuery, {
      //       type: sequelize.QueryTypes.SELECT,
      //       replacements: { id, startYYYYMM, endYYYYMM }
      //     });



      const [startMonth, startYear] = req.body.month.split("-");
      const [endMonth, endYear] = req.body.month.split("-");
    ...
    WHERE storage_center_id = :id
     ...
     `
      ...
      vaccineData = await sequelize.query(vaccineDataQuery, {
        type: sequelize.QueryTypes.SELECT,
        replacements: { id, startMonth, endMonth, startYear, endYear },
      });
    ...
    newData['vaccine_data'] = vaccineData;

    const totalQuantity = vaccineData.reduce((total, item) => total + item.total_stock, 0);
    newData['total_stock'] = totalQuantity;

    const totalReceived = vaccineData.reduce((total, item) => total + item.total_received, 0);
    newData['total_received'] = totalReceived;


    const totalOpeningBalance = vaccineData.reduce((total, item) => total + item.opening_balance, 0);
    newData['total_opening_balance'] = totalOpeningBalance;

    const totalUsableQty = vaccineData.reduce((total, item) => total + item.usable_qty, 0);
    newData['total_usable_qty'] = totalUsableQty;

    const totalIssued = vaccineData.reduce((total, item) => total + item.total_issued, 0);
    newData['total_issued'] = totalIssued;

    return Helper.response(true, "Data Found Successfully", newData, res, 200);

  } catch (error) {

    console.error("Error fetching vaccine warehouse by ID:", error);
